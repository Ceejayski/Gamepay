      ...
      <div className="d-flex flex-row-reverse py-2  pb-3 price-overview mx-2">
        <p className="m-0">
          {data.price_overview !== undefined ? (
            ...
            <>
              {(data.is_free && !data.release_date.coming_soon) && (
              <span className="free-span">
                Free to Play
              </span>
              )}
        ...
        <div className="mb-0 flex-grow-1" hidden={hover}>
          <AddToCartBtn id={data.steam_appid} text="Add to Cart" price={data.is_free || data.price_overview === undefined ? '$0' : data.price_overview.final_formatted} name={data.name} classes="light" />
        </div>
      </div>
    </div>
  );
